...
export async function addFavorite(snippetId: number, token: string) {
  try {
    ...
    throw new Error(err.response?.data?.message || err.message || "Erreur ajout favori");
  }
}
...
export async function removeFavorite(snippetId: number, token: string) {
  try {
    ...
    throw new Error(err.response?.data?.message || err.message || "Erreur suppression favori");
  }
}
