...
import { ref } from "vue";
import type { SessionUser, LoginInput, AuthResponse, User, SignupInput } from "../types/types";
...
token.value = response.data.token?.accessToken?.token || null;
    if (!token.value) throw new Error("Token manquant dans la réponse du serveur");
