...
export async function fetchAllTags(): Promise<Tag[]> {
  try {
    ...
    throw new Error(error.response?.data?.message || error.message || "Erreur lors de la récupération des tags");
  }
}
...
export async function fetchTagById(id: number): Promise<Tag> {
  try {
    ...
    const message =
      ...
      "Tag non trouvé" :
      error.response?.data?.message || error.message || "Erreur lors de la récupération du tag";
...
export async function createTag(tag: TagInput, token: string): Promise<Tag> {
  try {
    ...
    throw new Error(error.response?.data?.message || error.message || "Erreur lors de la création du tag");
  }
}

export async function updateTag(id: number, tag: TagInput, token: string): Promise<Tag> {
  try {
